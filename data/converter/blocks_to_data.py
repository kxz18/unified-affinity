...
def blocks_to_data(*blocks_list: List[List[Block]]):
    ...
    data = {
        'X': torch.tensor(X, dtype=torch.float),             # [Natom, 2, 3]
        'B': torch.tensor(B, dtype=torch.long),             # [Nb], block (residue) type
        'A': torch.tensor(A, dtype=torch.long),             # [Natom]
        'atom_positions': torch.tensor(atom_positions, dtype=torch.long),  # [Natom]
...
def blocks_to_data_simple(*blocks_list: List[List[Block]]):
    ...
    for i, blocks in enumerate(blocks_list):
        ...
        # other nodes
        for block in blocks:
            b = VOCAB.abrv_to_idx(block.abrv)
            x, a, positions, m, ca = [], [], [], [], []
            atom2node_id = {}
            for atom in block:
                atom2node_id[atom.name] = len(A) + len(cur_A) + len(a)
    ...
    data = {
        'X': torch.tensor(X, dtype=torch.float).unsqueeze(1),             # [Natom, 1, 3]
        'B': torch.tensor(B, dtype=torch.long),             # [Nb], block (residue) type
        'A': torch.tensor(A, dtype=torch.long),             # [Natom]
        'atom_positions': torch.tensor(atom_positions, dtype=torch.long),  # [Natom]
        'block_lengths': torch.tensor(block_lengths, dtype=torch.long),  # [Nresidue]
        'segment_ids': torch.tensor(segment_ids, dtype=torch.long),      # [Nresidue]
        'lengths': len(B)
        #'atom_mask': atom_mask,          # [Natom]
        #'is_ca': is_ca,                  # [Natom]
    }
