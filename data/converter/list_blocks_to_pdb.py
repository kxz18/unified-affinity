...
def list_blocks_to_pdb(list_blocks: List[List[Block]], chain_names: List[str], out_path: str) -> None:
    ...
    for blocks, chain_name in zip(list_blocks, chain_names):
        ...
        for i, block in enumerate(blocks):
            chain.add(_block_to_biopython(block, (i, ' ') if block.id is None else block.id))
        model.add(chain)
    structure.add(model)
    io = PDBIO()
...
def _block_to_biopython(block: Block, pos_code: tuple) -> BResidue:
    #_id = (' ', pos_code, ' ')
    _id = (' ', pos_code[0], pos_code[1] if len(pos_code[1]) else ' ') 
