...
def high_dim_scatterplot(data, vec, x='dim1', y='dim2', hue=None, labels=None, post_edit_func=None, save_path=None, output_reduced_data=None, downsample_ratio=None, **kwargs):
    ...
    if output_reduced_data is not None:
        output_reduced_data['x'] = reduced_data[x]
        output_reduced_data['y'] = reduced_data[y]
    if downsample_ratio is not None:
        idx = np.random.choice(X.shape[0], int(X.shape[0] * downsample_ratio), replace=False)
        for key in reduced_data:
            reduced_data[key] = [reduced_data[key][i] for i in idx]
...
def scatterplot(data, x, y, hue=None, labels=None, post_edit_func=None, save_path=None, **kwargs):
    ...
    if labels is not None:
        ...
        for i, label in enumerate(labels):
            if label is None:
                continue
            texts.append(ax.text(data[x][i], data[y][i], str(label), fontweight='bold'))
        adjust_text(texts)
